...
- Name: tier2-boundaries.spec.ts >> Tier 2: Boundary and Corner Cases >> 9. 3D Canvas Boundaries >> resizing window updates camera aspect ratio instantly
- Location: e2e\tier2-boundaries.spec.ts:416:5

# Error details

```
Error: locator.evaluate: Error: strict mode violation: locator('.right-column canvas') resolved to 2 elements:
    1) <canvas width="703" height="720" data-engine="three.js r185"></canvas> aka locator('canvas').first()
    2) <canvas width="659" height="800" data-engine="three.js r185"></canvas> aka locator('canvas').nth(1)
